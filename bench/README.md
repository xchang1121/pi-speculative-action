...
当前 `b638103` 修复了异步验证期间退休结果的迟到认领、持久历史失败丢失已封存证据、空闲目录退休未被关闭等待，以及 Pattern 前沿尚未准入就被跨轮跳过的问题。相同身份修订也确认准入；同会话动作回退只取权威样本，真实失配和匹配后拒绝更新原样本，淘汰、schema 替换、结束会话和复制令牌不污染现有反馈。原始 ephemeral 偶发断言仍无法归因：产物写入失败与事务时钟拒绝的独立注入均复现相同断言，当前诊断能区分其阶段、执行、作用域和可用证书，并在分支清理前保存验证及存储状态。它们不证明历史失败同源。
...
| 当前全开 | 9.929–10.040 | 34.862–34.873 | 22/23 |
