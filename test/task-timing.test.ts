...
describe("single-run serialized counterfactual timing", () => {
	it.each([
		...
		for (const [startedAt, completedAt] of actor) timeline.recordActor(startedAt!, completedAt!);
		for (const [startedAt, completedAt] of tools) timeline.recordTool({ startedAt: startedAt!, completedAt: completedAt! });
