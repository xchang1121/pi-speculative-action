...
describe("process provenance certificates", () => {
	...
	it("shares queue consumption across independent OFDs while retaining independent flags", () => {
		const graph: ProcessResourceGraph = { handles: [0, 3, 8].map(fd => ({ fd, description: fd === 3 ? 0 : fd })),
			...
			objects: { 0: { type: "pipe", contentDigest: sha256Digest("abc"), queue: { eof: false, bytes: 3, capacity: 4096, producer: "live" } } } };
	...
	it.each(["regular", "null", "directory", "pipe", "socket"] as const)("seals OFD state separately from shared object effects (%s)", type => {
