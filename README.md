...
继承资源统一表示为进程 FD 表 → OFD → 内核对象：FD 复制、关闭、CLOEXEC 和受控进程树内传递保留引用关系；OFD 保存共享偏移和状态标志，文件保存内容，管道和 Unix stream socket 保存队列、两端及生产者状态。普通文件、null、目录/O_PATH、匿名字节管道和内部 socketpair 共用捕获、隔离执行、运行中借用、一次性移交及取消回收流程。SCM_RIGHTS/批量消息和 pidfd_getfd 使用原生内核身份证明；内部消息中最后来源已关闭的句柄可从完整的对象来源恢复所有权，未知外部导入、导出不能授予移交权。

活跃流保留已捕获输入及真实 EOF/半关闭状态，支持别名、两端队列、并发收发和运行中结果等待。消费、窥读、写入和半关闭使用同一有序操作记录，整树暂停后先验证完整状态，再提交一次；提交后失败会终止进程树，避免重复执行副作用。只读管道继续使用原有消费差分，不增加读写调用跟踪。当前流移交有界：输入最多 2 MiB，每端输出最多 4096 字节、16 次写入，操作记录最多 1024 条；写入还须满足实际内核容量。未捕获的未来输入不会被猜测，输入不足的提前执行等待或取消，不发布结果。外部网络、包式队列、附带控制消息的流重放和无法证明的资源状态保留原生执行。标准输出/错误仍使用既有缓冲路由。此版本使用 helper 协议 19、FD2 清单和证书版本 9，旧 helper 需重新构建。
