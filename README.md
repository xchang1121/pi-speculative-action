...
Linux 后端可在同一会话内保留已封存子进程的执行绑定，在新隔离工作区中重新执行该子进程。PatternAware 从真实采纳记录学习，在空闲容量的根候选中提前执行已绑定的内部单元；父 Bash 仍独立执行和验证。一次性结果消费后可保留绑定，重新执行产生新证据，旧结果不能再次转交。原始参数和环境仅驻留内存，受条目数及最多 4 MiB 的绑定预算约束，清理缓存或关闭后撤销。预执行重新检查父动作权限；实际子进程采纳仍验证完整执行身份和当前依赖，内部命中单列统计。
