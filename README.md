...
- 输入总预算 8 MiB、结果预算 1 MiB；首次捕获时可在原资源预算内保留私有搜索目录，同一路径与 glob 的新搜索词可复用筛选和目录准备，仍执行当前查询并验证原始依赖。预算不足或准备身份不符时按需重建；目录随资源版本回收，取消单个查询不撤销其他消费者的输入。取消和关闭等待工作进程及借用的输入操作结束。这是固定可信工具执行器，不是任意 JavaScript 沙箱。
...
- 自定义后端用 `inputResources` 提供封存输入的绝对名称提示，动作的 `resourceRoot` 用于解析逻辑资源名称。`reconstruct` 默认只支持源执行器；声明 `reconstructionScope: "current_action"` 后可尝试当前动作，并返回 `{ output, compatibility, validate, capturedBytes? }`。当前执行器证明仅随查询验证能力传递，额外索引和证明存储计入原缓存预算。查询证明超预算时可保留结果并回到完整输入验证。输入检索取代旧 `RESOURCE_INPUT_ACTION_KEY_PROJECTOR`，`inputs` 采纳单列统计，不算作预测匹配或整项结果投影。
...
任务主加速比为同次运行的 `serializedCounterfactualMs / actualEndToEndMs`：反事实保留实际开销，只移除权威计算的重叠，并按计算身份去重。整项结果与结果投影可计入来源计算；封存输入上的重算只计当前查询，不能把来源工具未使用的结果计算计为节省。无独立计算证据的输入预热只按实际成本举证。无重叠为 1×，有重叠大于 1×；开启投机不另跑一条真实 Actor 串行路径来生成主基线。
