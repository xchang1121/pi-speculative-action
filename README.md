...
开启历史模式的多步预测后，PatternAware 可以接续 Drafter 已完成的根工具批次，提前绑定后续步骤的参数。整批结果仅作为假设上下文，父子预测分别结算；只有真实 Actor 批次进入历史学习。文本候选列表保留返回顺序，旧格式的模式状态自动重新学习。联动沿用深度、并发、依赖验证和取消限制，不额外发起模型请求。
...
端点、传输方式、Actor Profile、置信度和预算在 TUI 配置。Profile 必须与 Actor 实际模板和 tokenizer 一致；认证使用 `apiKeyEnv`。目标端 token 验收与工具采纳分别统计，候选注册确认不算 token 验收。
...
- 运行中接管和一次性结果有独立所有权。父分支与子进程不能重复消费；关闭排空执行、借用输入、封存、事务和清理。
