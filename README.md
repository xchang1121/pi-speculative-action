...
默认两个 Drafter 请求竞争首个有效提案，仍在运行的同伴会被取消；更大的候选宽度增加模型请求成本。一个响应中的工具批次保持完整，整批结果反馈后才续推。`drafterMaxDepth: 0` 关闭 Drafter 自身续推；收益门控默认开启：Drafter 请求与 Actor 并行，只把采纳延迟计为成本，以同一会话内各提示的采纳收益决定是否继续起草。工具列表只限制预测，不改变 Actor 的正常工具权限。
...
实际端到端时延与估计收益分开报告，乐观与净值两种口径并列：`optimisticAccelerationRatio = 1 + optimisticSavingsMs / actualEndToEndMs`，净值 `1 + estimatedSavingsMs / actualEndToEndMs`，多次运行先累加分子和分母。乐观收益（`optimisticSavingsMs`）沿用原口径：每次成功采纳以已有计算区间（含复用的子进程）与 Actor 预期耗时中较大者为参考，扣除本次实际等待、验证和采纳时间，不为负、不扣任何投机开销。净收益（`estimatedSavingsMs`）带符号：每次成功采纳以调度器同操作（或同类）原生执行耗时中位数为参考（不含可选捕获与结算时间；无原生样本时用已有计算区间，含复用的子进程），扣除本次实际等待、验证和采纳时间，慢于参考即记为负；每次原生执行另扣除投机在其路径上的开销（匹配、失败的等待、捕获准备与结算）；跨任务缓存和重复采纳分别计入。原生父进程只补入被复用的子计算，扣除其已经包含的计算或等待区间，不把普通子进程并发算作投机收益。只遍历已有区间，不添加计时后端、历史库或额外执行。

TUI 页脚和状态明细显示 `End-to-End SpeedUp +12.1% (net +3.4%)`，前者为 `(optimisticAccelerationRatio - 1) × 100%`，括号内为净收益口径；状态明细与任务事件同样并列显示乐观与净收益时长。页脚按累计已完成任务的总时间计算，并显示实际端到端时延；尚无已完成任务或旧记录缺少数据时显示 `n/a`。

这是估计，不是关闭投机后的实测节省：学习模式下的原生执行本身若被观察放慢，参考值也随之偏高；历史样本、来源计算与当前原生执行可能有差异。封存输入上的重算只用当前查询或同操作原生历史，不计来源工具未使用的计算；两者均无证据时不猜测预热收益。原有 `hiddenLatencyMs` 和 `serializedCounterfactualMs / actualEndToEndMs` 保留用于比较计算重叠；后者按计算身份去重、不计前一任务的计算，不能覆盖全部复用收益。新旧口径不可混作性能提升。

TUI 同时显示 `Tool time speed up 62.5%`：工具执行时间被掩盖的比例 `hiddenLatencyMs / toolExecutionMs × 100%`，累计时先累加分子、分母；状态明细和任务事件另显示如 `100ms of 160ms tool time hidden`。直接复用已采纳计算区间的重叠计时，无工具执行时间时显示 `n/a`。它统计工具与 Actor 阶段或其他工具的重叠，但相互重叠的原生执行（并行批次）只计其并集，按计算身份去重且不计前一任务的计算；跨任务缓存免执行收益仍由端到端收益口径反映。
