...
TUI 同时显示 `Tool time speed up 62.5%`：工具执行时间被掩盖的比例 `hiddenLatencyMs / toolExecutionMs × 100%`，累计时先累加分子、分母；状态明细和任务事件另显示如 `100ms of 160ms tool time hidden`。直接复用已采纳计算区间的重叠计时，无工具执行时间时显示 `n/a`。它统计工具与 Actor 阶段或其他工具的重叠，按计算身份去重且不计前一任务的计算；跨任务缓存免执行收益仍由端到端收益口径反映。
