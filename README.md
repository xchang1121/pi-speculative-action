...
- 原版 Pi 负责参数与输出语义。同工具、同资源集合的新查询由原执行器在封存输入上求值；可追溯到原始观察时，只验证本次使用的输入，保留路径与别名证明，否则验证完整输入。查询证明不授权原结果提交。结果封存复用已有证据，并发加入同一候选共用正在进行的验证，采纳时独立精确验新，原生工具观察仍须封闭执行窗口。默认不使用 read 范围裁剪，也不从命令文本猜测等价性。
- 自定义执行后端的 `reconstruct` 返回 `{ output, validate?, capturedBytes? }`；可选证明只授权当前无效果查询，额外证明存储计入缓存预算。证明超预算时可保留查询结果并回到完整输入验证。
