...
- Git 快照保留原始字节，独立管理索引、配置和属性；不继承用户 hook、过滤器或索引路径。过滤后的私有快照只采集内容验新所需证据。独立子进程可借用同一工作区池已有的不可变准备结果，省去启动前再次整树验新；准备结果不授予采纳权，当前输入和效果仍须通过完整证明。普通分叉保持精确验新。
