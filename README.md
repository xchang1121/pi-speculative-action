...
实际端到端时延与估计收益分开报告：`optimisticAccelerationRatio = 1 + estimatedSavingsMs / actualEndToEndMs`，多次运行先累加分子和分母。收益带符号：每次成功采纳以调度器同操作（或同类）原生执行耗时中位数为参考（不含可选捕获与结算时间；无原生样本时用已有计算区间，含复用的子进程），扣除本次实际等待、验证和采纳时间，慢于参考即记为负；每次原生执行另扣除投机在其路径上的开销（匹配、失败的等待、捕获准备与结算）；跨任务缓存和重复采纳分别计入。原生父进程只补入被复用的子计算，扣除其已经包含的计算或等待区间，不把普通子进程并发算作投机收益。只遍历已有区间，不添加计时后端、历史库或额外执行。
