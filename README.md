...
原生程序可以通过 CPU 指令或 ELF 启动状态取得时钟和随机输入，系统调用观察不能证明它们未被使用。当前 Linux 后端统一保留这些输入限制：只允许同轮内转交一次已执行或仍在运行的计算，不将其发布为可跨轮、跨会话重放的历史结果；旧观察契约的证书拒绝采纳。可执行绑定仍可跨轮保留，在新轮次重新预执行。更细的原生结果重放仍需补足实例输入证明。
